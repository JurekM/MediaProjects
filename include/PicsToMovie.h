...
public:
   ...
   bool initialize(const wchar_t* outfile);
   bool configure(unsigned width, unsigned height);
   bool addFrame(const wchar_t* paths);
   bool addFrameWildcard(const wchar_t* wildcard);
   bool finalize();
...
protected:
   bool initializeSinkWriter(IMFSinkWriter** ppWriter, DWORD* pStreamIndex, const wchar_t* path, unsigned width, unsigned height);
   bool writeFrame(IMFSinkWriter* pWriter, DWORD streamIndex, const LONGLONG& rtStart, COLORREF* frame, unsigned width, unsigned height);
   ...
   DWORD _streamIndex{};
   LONGLONG _rtStart{};
   CComPtr<IMFSinkWriter> _pSinkWriter{};
   std::vector<COLORREF> _frame{};
};
